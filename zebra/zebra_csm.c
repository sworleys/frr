...
{
	thread_add_event(zrouter.master, zebra_csm_fast_restart,
			 NULL, false, NULL);
}
 ...
 */
static void frr_csm_handle_up_down_trigger(Module mod, Mode mode,
					   State state, bool up)
{
	if (up) {
...
static int frr_csm_cb(int len, void *buf)
{
	...
	while (nbytes && nbytes >= entry->len) {
		...
		case COME_UP:
			mod_mode = (module_mode *)entry->data;
			zlog_debug("FRRCSM: Received ComeUp for %s, mode %s state %s",
				   mod_id_to_str(mod_mode->mod),
				   mode_to_str(mod_mode->mode),
				   mod_state_to_str(mod_mode->state));
			frr_csm_handle_up_down_trigger(mod_mode->mod,
				mod_mode->mode, mod_mode->state, true);
			break;
		case GO_DOWN:
			mod_mode = (module_mode *)entry->data;
			zlog_debug("FRRCSM: Received GoDown for %s, mode %s state %s",
				   mod_id_to_str(mod_mode->mod),
				   mode_to_str(mod_mode->mode),
				   mod_state_to_str(mod_mode->state));
			frr_csm_handle_up_down_trigger(mod_mode->mod,
				mod_mode->mode, mod_mode->state, false);
			break;
		case UP:
			mod_status = (module_status *)entry->data;
			mod_mode = &mod_status->mode;
			zlog_debug("FRRCSM: Received Up for %s, mode %s State %s fr %d",
				   mod_id_to_str(mod_mode->mod),
				   mode_to_str(mod_mode->mode),
				   mod_state_to_str(mod_mode->state),
		...
		case DOWN:
			mod_mode = (module_mode *)entry->data;
			zlog_debug("FRRCSM: Received Down for %s, mode %s state %s",
				   mod_id_to_str(mod_mode->mod),
				   mode_to_str(mod_mode->mode),
				   mod_state_to_str(mod_mode->state));
			frr_csm_update_state(mod_mode->mod, mod_mode->mode,
		...
		case KEEP_ALIVE_REQ:
			...
			// zlog_debug("... Received Keepalive Req, seq %d", kr->seq);
			frr_csm_send_keep_rsp(kr->seq);
			break;
		case NETWORK_LAYER_INFO:
			mod_status = (module_status *)entry->data;
			mod_mode = &mod_status->mode;
			zlog_debug("FRRCSM: NL Info for %s, mode %s State %s fr %d",
				   mod_id_to_str(mod_mode->mod),
				   mode_to_str(mod_mode->mode),
				   mod_state_to_str(mod_mode->state),
				   mod_status->failure_reason);
		...
		case MODULE_STATUS_REQ:
			zlog_debug("FRRCSM: Received ModStatus Req");
			frr_csm_send_state();
			break;
		default:
			/* Right now, we don't care about anything else */
			zlog_debug("FRRCSM: Received unhandled message %d",
				   entry->type);
			break;
		}
		nbytes -= entry->len;
