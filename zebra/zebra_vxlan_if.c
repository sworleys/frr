...
static int zebra_vxlan_if_vni_entry_update(struct zebra_if *zif,
	struct zebra_vxlan_vni *old_vni, struct zebra_vxlan_vni *new_vni,
	uint16_t chgflags)
{
	if (!chgflags)
		return 0;
	...
	if (old_vni->access_vlan != vni->access_vlan) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("vxlan %s updating vni(%d, %d) -> vni(%d, %d)",
				...
				vni_tmp.vni, vni_tmp.access_vlan);

		zebra_vxlan_if_vni_entry_update(zif, old_vni, &vni_tmp, ZEBRA_VXLIF_VLAN_CHANGE);
...
struct zebra_vxlan_vni * zebra_vxlan_if_access_vlan_find(struct zebra_if *zif,
	uint8_t vlan_aware, vlanid_t vid)
{
	struct zebra_vxlan_vni *vni = NULL;
	struct zebra_vxlan_if_vlan_ctx ctx;
	...
	ctx.vid = vid;
	ctx.vni = NULL;
	zebra_vxlan_if_vni_walk(zif, zebra_vxlan_if_vni_access_vlan_find, (void *)&ctx);
	vni = ctx.vni;
	...
	vni_t vni_id, struct in_addr *mcast_group)
{
	...
	return zebra_vxlan_if_vni_update(ifp, &vni, ZEBRA_VXLIF_MCAST_GRP_CHANGE);
}
